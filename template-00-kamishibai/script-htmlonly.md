<script>
// すべてのスライド(section)から scene を抜く
const sceneList = Array.from(document.querySelectorAll('section')).map((slide, index) => {
  return {
    page: index + 1,
    scene: slide.dataset.scene || "none",
    title: slide.querySelector('h1,h2,h3')?.textContent?.trim() ?? null,
  };
});
